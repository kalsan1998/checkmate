...
class ChessBoard{
	...
	King &getKing(Colour colour) const;
	...
	bool isCheck(Colour turn) const; //returns true if turn is in check
	bool isStalemate(Colour turn) const; //returns true if turn has no legal moves left
	bool isCheckmate(Colour turn) const; //returns true if the person whose turn it is is checkmated
